...
void generate_level(lander_game_state_t *g_state){
    ...
    do{
        g_state->pad.pos_x = 10 + ((unsigned char)quick_rand(66))%105;
        g_state->pad.pos_y = 30 + ((unsigned char)quick_rand(67))%90;
...
void lander_update()
{
    ...
    lander_game.v_slide = G_touch_pct; //getVerticalPosition();
    if(LEFT_BTN)
        lander_game.h_slide = 10;
    else if (RIGHT_BTN)
        lander_game.h_slide = 80;
    else
        lander_game.h_slide = 50;
    ...
    //if(LEFT_BTN && lander_game.lander_ship_fuel){    
        lander_game.ship.acc_x -= 1;
        lander_game.lander_ship_fuel -= FUEL_USAGE;
    }
    else if(lander_game.h_slide > 70 && lander_game.lander_ship_fuel){
    //else if(RIGHT_BTN && lander_game.lander_ship_fuel){
        lander_game.ship.acc_x += 1;
        lander_game.lander_ship_fuel -= FUEL_USAGE;
    }
